...
const Slider = forwardRef<SliderRef, PropsWithChildren<SliderProps>>((props, ref) => {
  ...
  return (
    <sliderContext.Provider value={{ ...propsWithoutChildren, childrenCount, sliderId: useId() }}>
      <div
        className={clsx('crow:overflow-hidden crow:relative crow:group', className)}
        style={{ direction: 'rtl' }}
      >
        <div
          className={clsx(
            'crow:flex crow:snap-x crow:snap-mandatory crow:overflow-x-auto crow:no-scrollbar',
            containerClassName,
          )}
