...
const Select = <T,>(props: SelectProps<T>) => {
  ...
  if (wrapperProps.dropdownType === 'popover' && !searchable)
    wrapperProps.popoverClassName = clsx('crow:pt-3', wrapperProps.popoverClassName);
  if (wrapperProps.dropdownType === 'drawer')
    wrapperProps.drawerProps = {
      ...wrapperProps.drawerProps,
      containerClassName: clsx(
        searchable ? 'crow:!pt-0' : 'crow:!pt-3',
  ...
  if (!wrapperProps.customInput)
    wrapperProps.inputProps = {
      ...
      prefixClassName: hasMultipleValues
        ? clsx(
            'crow:flex crow:items-center flex crow:flex-wrap crow:gap-2 crow:w-full',
            wrapperProps.inputProps?.prefixClassName,
          )
        : wrapperProps.inputProps?.prefixClassName,
      className: hasMultipleValues ? 'crow:hidden' : wrapperProps.inputProps?.className,
      containerClassName: hasMultipleValues
        ? clsx('crow:items-baseline', wrapperProps.inputProps?.containerClassName)
